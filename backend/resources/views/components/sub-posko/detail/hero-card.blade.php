        ...
        <div class="flex-1 space-y-4 w-full">
            <div class="flex flex-wrap items-center gap-2.5">
                <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold 
                    ...
                    <span class="w-1.5 h-1.5 rounded-full {{ $subPosko->status == 'aktif' ? 'bg-emerald-500' : ($subPosko->status == 'siaga' ? 'bg-amber-500' : 'bg-rose-500') }}"></span>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 pt-3 border-t border-slate-100 text-xs">
                <div>
                    ...
                    <div class="inline-flex items-center gap-1.5 bg-indigo-50 text-indigo-700 px-2.5 py-1 rounded-lg border border-indigo-100 font-mono font-bold">
                        <span>{{ $subPosko->kode_undangan }}</span>
                        <button onclick="navigator.clipboard.writeText('{{ $subPosko->kode_undangan }}'); alert('Kode Akses Berhasil Disalin!');" title="Salin Kode" class="text-indigo-500 hover:text-indigo-800 transition">
                ...
                <div>
                    ...
                    <strong class="text-slate-800 font-semibold mt-1 block">{{ $subPosko->bencana->jenis_bencana ?? 'Bencana Umum' }}</strong>
                </div>

                <div>
                    <span class="text-slate-400 font-medium block">Penanggung Jawab</span>
                    <strong class="text-slate-800 font-semibold mt-1 block">{{ $subPosko->penanggung_jawab ?? '-' }}</strong>
                    <span class="text-[11px] text-slate-400">{{ $subPosko->kontak_hp ?? '-' }}</span>
                </div>

                <div>
    <span class="text-slate-400 font-medium block">Petugas</span>
    <strong class="text-slate-800 font-semibold mt-1 block">{{ $subPosko->jumlah_petugas ?? 0 }} Orang</strong>
</div>

                <div>
                    <span class="text-slate-400 font-medium block">Dibuat Pada</span>
                    <strong class="text-slate-800 font-semibold mt-1 block">{{ $subPosko->created_at ? $subPosko->created_at->translatedFormat('d M Y') : '-' }}</strong>
                    <span class="text-[11px] text-slate-400">{{ $subPosko->created_at ? $subPosko->created_at->format('H:i') . ' WIB' : '' }}</span>
                </div>
            </div>
        </div>
    </div>
