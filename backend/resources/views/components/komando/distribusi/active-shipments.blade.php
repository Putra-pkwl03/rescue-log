  ...
  <div class="space-y-3 max-h-[620px] overflow-y-auto pr-1">
        @forelse($pengirimans->where('status_pengiriman', 'dalam_perjalanan') as $shipment)
            <div class="p-4 rounded-xl border border-slate-200 bg-slate-50/50 hover:bg-white hover:border-blue-200 hover:shadow-sm transition-all space-y-3">
                <div class="flex items-start justify-between gap-2">
                    <div>
                        ...
                        <h4 class="font-bold text-slate-900 text-sm mt-1">
                            Tujuan: {{ $shipment->poskoTujuan->nama_posko ?? $shipment->pengajuan->posko->nama_posko ?? 'Posko Lapangan' }}
                        </h4>
                    </div>
                ...
                <!-- Tombol Interaktif ke Peta -->
                @php
                    $latAsal = $shipment->lat_asal ?? -7.7956;
                    $longAsal = $shipment->long_asal ?? 110.3695;
                    $latTujuan = $shipment->lat_tujuan ?? $shipment->pengajuan->posko->latitude ?? -7.7970;
                    $longTujuan = $shipment->long_tujuan ?? $shipment->pengajuan->posko->longitude ?? 110.3700;
                    $namaPosko = $shipment->poskoTujuan->nama_posko ?? $shipment->pengajuan->posko->nama_posko ?? 'Posko Tujuan';
                @endphp
