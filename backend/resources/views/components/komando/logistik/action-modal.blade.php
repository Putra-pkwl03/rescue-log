...
<div id="actionModal" class="fixed inset-0 z-50 hidden bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-100 transform transition-all space-y-4">
        ...
        <div class="flex items-center justify-between border-b border-slate-100 pb-3">
            ...
            <button type="button" onclick="closeActionModal()" class="text-slate-400 hover:text-slate-600 cursor-pointer">
        ...
        <div class="space-y-2.5 pt-2">
            <!-- 1. Setujui Penuh (PATCH) -->
            <form id="formApproveFull" method="POST" action="">
                @csrf
                @method('PATCH') <!-- TAMBAHKAN DI SINI -->
            ...
            <!-- 3. Tolak Pengajuan (PATCH) -->
            <form id="formReject" method="POST" action="" onsubmit="return confirm('Apakah Anda yakin ingin MENOLAK pengajuan logistik ini?');">
                @csrf
                @method('PATCH') <!-- TAMBAHKAN DI SINI -->
        ...
        <div class="pt-3 border-t border-slate-100 flex justify-end">
            <button type="button" onclick="closeActionModal()" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 rounded-xl text-slate-700 text-xs font-semibold transition-colors cursor-pointer">
                Batal
            </button>
        </div>
    </div>
