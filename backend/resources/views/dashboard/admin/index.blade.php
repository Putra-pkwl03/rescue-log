        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div>
                @if($posko->status === 'terdaftar_nonaktif')
                    <a href="{{ route('admin.bencana.aktifkan.form') }}" 
                       class="inline-flex items-center bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors shadow-sm">
                        Aktifkan Posko
                    </a>
                @elseif($bencanaAktif)
                    <form action="{{ route('admin.bencana.nonaktifkan') }}" method="POST">
                        @csrf
                        <button type="submit" 
                                onclick="return confirm('Apakah Anda yakin ingin me-nonaktifkan posko ini?')"
                                class="inline-flex items-center bg-red-600 hover:bg-red-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors shadow-sm">
                            Nonaktifkan Posko
                        </button>
                    </form>
                @endif
...
<div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
    ...
    <ul class="space-y-4">
        ...
        <li class="flex items-start gap-3">
            <span class="w-2.5 h-2.5 bg-red-500 rounded-full mt-1.5 shrink-0"></span>
            <div>
                <p class="text-sm font-semibold text-gray-800">Peringatan Sistem</p>
                <p class="text-xs text-gray-600 mt-0.5">Stok Beras Premium di Gudang Utama tersisa kurang dari 50 Karung.</p>
                <p class="text-[11px] text-gray-400 mt-1">1 jam yang lalu</p>
            </div>
        </li>
        <li class="flex items-start gap-3">
            <span class="w-2.5 h-2.5 bg-blue-500 rounded-full mt-1.5 shrink-0"></span>
            <div>
                <p class="text-sm font-semibold text-gray-800">Pos Komando Baru</p>
                <p class="text-xs text-gray-600 mt-0.5">Pos Komando Bencana Tanah Longsor resmi diaktifkan oleh Admin.</p>
                <p class="text-[11px] text-gray-400 mt-1">3 jam yang lalu</p>
            </div>
        </li>
    </ul>
</div>
